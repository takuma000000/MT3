...
void DrawBezier(const Vector3& controlPoint0, const Vector3& controlPoint1, const Vector3& controlPoint2,
	...
	for (int i = 0; i < numSegments; ++i) {
		...
		float nextT = (i + 1) / static_cast<float>(numSegments);

		// 現在の t での点を計算
		Vector3 p0 = Lerp(Lerp(controlPoint0, controlPoint1, t), Lerp(controlPoint1, controlPoint2, t), t);
		// 次の t での点を計算
		Vector3 p1 = Lerp(Lerp(controlPoint0, controlPoint1, nextT), Lerp(controlPoint1, controlPoint2, nextT), nextT);

		// ビュープロジェクション行列を使用してポイントを変換
		p0 = Transform(p0, viewProjectionMatrix);
		p1 = Transform(p1, viewProjectionMatrix);

		// ビューポート行列を使用してポイントをスクリーン座標に変換
		p0 = Transform(p0, viewportMatrix);
		p1 = Transform(p1, viewportMatrix);

		// 2つのポイント間に線を描画
		Novice::DrawLine(int(p0.x), int(p0.y), int(p1.x), int(p0.y), color);
	}
