...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	pendulum.angle = 0.7f;
	pendulum.anglarVel = 0.0f;
	pendulum.anglarAcc = 0.0f;
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (isMoving) {
			pendulum.anglarAcc = -(9.8f / pendulum.length) * std::sin(pendulum.angle);
			pendulum.anglarVel += pendulum.anglarAcc * time;
			pendulum.angle += pendulum.anglarVel * time;
