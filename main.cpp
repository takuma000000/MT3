...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (isMoving) {
			co.anglarVel = std::sqrt(9.8f / (co.length * std::cos(co.half)));
			co.angle += co.anglarVel * time;

			float radius = std::sin(co.half) * co.length;
			float height = std::cos(co.half) * co.length;

			sphere.center.x = co.anchor.x + radius * std::cos(co.angle);
			sphere.center.y = co.anchor.y - height;
			sphere.center.z = co.anchor.z + radius * std::sin(co.angle);
		}
		...
		DrawLine(start, sphere.center, viewProjectionMatrix, viewportMatrix, WHITE);
